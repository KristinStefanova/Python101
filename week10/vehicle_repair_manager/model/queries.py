...
insert_service = """
INSERT OR REPLACE INTO service(name)
VALUES (:name)
"""
...
find_repair_hour_mechanic_info = """
SELECT R.date,  R.start_hour, V.user_name, V.make, V.model, R.bill
FROM repair_hour AS R
JOIN (SELECT V.id, V.make, V.model, O.user_name
        FROM vehicle AS V
        JOIN (SELECT C.base_id, B.user_name
                FROM client AS C
                JOIN base_user AS B
                ON B.id = C.base_id) AS O
        ON V.owner = O.base_id) AS V
ON V.id = R.vehicle
WHERE R.id = ?
"""
